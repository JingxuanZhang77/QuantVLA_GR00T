...
class DuQuantConfig:
    ...
    weight_bits: int = int(os.environ.get("GR00T_DUQUANT_WBITS_DEFAULT", 4))
    act_bits: int = int(os.environ.get("GR00T_DUQUANT_ABITS", 8))
    block_size: int = int(os.environ.get("GR00T_DUQUANT_BLOCK", 16))
    lambda_smooth: float = float(os.environ.get("GR00T_DUQUANT_LS", 0.15))
    enable_permute: bool = os.environ.get("GR00T_DUQUANT_PERMUTE", "1") not in ("0", "false", "False")
    act_percentile: float = float(os.environ.get("GR00T_DUQUANT_ACT_PCT", 99.9))
    calib_batches: int = int(os.environ.get("GR00T_DUQUANT_CALIB_STEPS", 32))
    pack_dir: Optional[str] = os.environ.get("GR00T_DUQUANT_PACKDIR", None)
    row_rot_mode: str = os.environ.get("GR00T_DUQUANT_ROW_ROT", "restore")  # values: '0', 'restore', 'propagate'
    block_out_size: int = int(os.environ.get("GR00T_DUQUANT_BLOCK_OUT", os.environ.get("GR00T_DUQUANT_BLOCK", 16)))
